	...
	{
		$this->db->select("usuarios.id_usuario, usuarios.nome, usuarios.login, usuarios.email");
	...
	public function getInfo($login, $password)
	{
		...
		$this->db->select("id_usuario, nome, login, email");
	...
	{
		if (! $usuario || empty($usuario['nome']) || empty($usuario['login']) || empty($usuario['email']))
		{
			$result = array('warning' => '<p>Todos os campos devem ser preenchidos para atualizar o usuário!</p>');
			return $result;
		}
	...
	public function isLogged ()
	{
		...
		if (isset($user_logged['login']) && $user_logged['login'] != null && $user_logged['tipo'] == 1) {
			return true;
		}
	...
	public function passRecoverCreate ($userid=null)
	{
		...
		$mensagem .= "<p class='p_mail'><b>De: </b> Atacadão de Veículos</p>";
		...
		$this->email->from("contato@atacadaoveiculos.com.br");
		$this->email->to($user->email);
		$this->email->subject("Recuperação de Senha - Atacadão de Veículos");
	...
	// 	CAST(
	// 	(SELECT
	// 	GROUP_CONCAT(usuarios.id_usuario) AS online
	// 	FROM
	// 	usuarios
	// 	JOIN sessions
